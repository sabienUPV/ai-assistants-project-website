...
&lt;/IfModule&gt;</code></pre></li>';
    ...
    if ( $table_exists ) {
        ...
        // --- TOP COUNTRIES ---
        // Note: We use NULLIF to merge NULL and empty strings ('') into a single group
        $top_countries = $wpdb->get_results($wpdb->prepare("
            SELECT NULLIF(country, '') as country, COUNT(DISTINCT CONCAT(visitor_hash, FLOOR((visit_timestamp + %d) / 86400))) as visits
            FROM $table_name
            WHERE visit_timestamp >= %d
            GROUP BY NULLIF(country, '')
            ORDER BY visits DESC
            LIMIT 10
        ", $wp_offset_seconds, $filter_timestamp));
        ...
        if ( $top_countries ) {
            foreach ( $top_countries as $row ) {
                if ( empty( $row->country ) ) {
                    ...
                    // Grey placeholder for unknown country (instead of a flag)
                    echo '<div style="width: 24px; height: 16px; background: #e2e4e7; border-radius: 2px; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; color: #8c8f94; box-shadow: 0 0 2px rgba(0,0,0,0.3);">?</div>';
                    echo '<span><strong>Unknown</strong> <span style="color: #646970; font-size: 12px;">(N/A)</span></span>';
                    echo '</div>';
                    echo '</td>';
...
function ai4pid_render_markdown_export_ui($table_name, $wp_offset, $filter_timestamp, $period_label, $metrics) {
    ...
    // Note: We use NULLIF to merge NULL and empty strings ('') into a single group for Markdown export
    $top_countries = $wpdb->get_results($wpdb->prepare("SELECT NULLIF(country, '') as country, COUNT(DISTINCT CONCAT(visitor_hash, FLOOR((visit_timestamp + %d) / 86400))) as uniques FROM $table_name WHERE visit_timestamp >= %d GROUP BY NULLIF(country, '') ORDER BY uniques DESC LIMIT 10", $wp_offset, $filter_timestamp));
    
    $md .= "### Top Countries ({$period_label} Visitor-Days)\n";
    $md .= "| Country | Country Code | Visitor-Days |\n|---|---|---|\n";
    if ($top_countries) {
        foreach ($top_countries as $c_row) { 
            $c_name = empty($c_row->country) ? 'Unknown' : ai4pid_get_country_name($c_row->country);
            $c_code = empty($c_row->country) ? 'N/A' : strtoupper($c_row->country);
            $md .= "| " . $c_name . " | " . $c_code . " | " . intval($c_row->uniques) . " |\n"; 
        }
    }
